...
var padding = 5  # Extra padding be aat grid level
...
func _draw():
	for i in range(1, 3):  # Draw at 3rd and 6th positions
		...
		draw_rect(Rect2(
			Vector2(offset_x - thickness / 2, -padding + line_reduction_v), 
			Vector2(thickness, cell_size.y * 9 + 2 * padding - 2 * line_reduction_v + vertical_extend)  
		), Color.BLACK)

		# Horizontal lines with shorter width
		draw_rect(Rect2(
			Vector2(-padding + line_reduction_h, offset_y - thickness / 2),  
			Vector2(cell_size.x * 9 + 2 * padding - 2 * line_reduction_h, thickness) 
		), Color.BLACK)
